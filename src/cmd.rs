...
#[derive(Debug)]
pub struct CmdBuilder<'source, 'sink> {
    cmd: Command,
...
impl<'source, 'sink> CmdBuilder<'source, 'sink> {
    ...
    pub fn build(self) -> Cmd<'source, 'sink> {
        Cmd::new(self.cmd, self.source, self.sink)
    }
}
...
impl Deref for Cmd<'_, '_> {
    type Target = Command;
    fn deref(&self) -> &Self::Target {
        &self.cmd
    }
}
impl DerefMut for Cmd<'_, '_> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.cmd
    }
}

impl<'source, 'sink> Cmd<'source, 'sink> {
